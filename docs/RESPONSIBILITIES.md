...
本文是 `framework-starter` 工程的项目文档入口，只约束 `framework-starter` 工程及其 `framework-starter-*` module 的内部设计；服务工程按需引用 starter。
...
- 提供技术中立的 `IntegrationEventOutboxEnvelope`、持久化 repository 契约和 `direct`、`reliable` 两种模式；具体持久化技术在各自实现 module 中定义 DO、DO 转换 mapper 和 repository 实现。
- 持久化转换契约以 `*PersistenceMapper` 声明，MapStruct 实现放在契约包的 `mapstruct` 子包并命名 `*MapStructMapper`；`domain-eventstore` 采用相同结构。
